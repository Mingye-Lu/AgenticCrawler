...
Eleven crates under `crates/`, compiled with `resolver = "2"` (10 application crates + 1 transitional shim):

- **core** (`acrawl-core`) — shared types, traits, and error hierarchy used across the workspace. Defines `ToolSpec`, `ToolEffect`, `AssistantEvent`, `RuntimeObserver`, `Message`/`MessageContent`, and the `OutputOutcome` enum.
...
- **runtime** — `ConversationRuntime` (the core turn loop), `Session` persistence, system-prompt builder, compaction, usage/pricing, OAuth PKCE, `config/` subdirectory (loader, MCP config, features), and a full MCP client stack in `mcp/` (`client.rs`, `types.rs`, `server_manager.rs`, `process.rs`, `naming.rs`).
...
- **crawler** _(transitional shim)_ — re-exports from `agent`, `browser`, and `core` for backward compatibility. Contains no logic of its own; will be removed once all downstream references migrate to the new crate names.
