...
impl StdoutSink {
    ...
    #[must_use]
    pub fn with_is_tty(is_tty: bool) -> Self {
        Self {
    ...
    fn print_tool_line(&self, line: &ToolLine, color: Option<Color>) {
        if self.is_tty {
            let mut stdout = io::stdout();
    ...
    pub fn take_pending_tool(&self, name: &str) -> Option<(usize, usize, String)> {
...
#[cfg(test)]
mod tests {
    use super::{OutputSink, StdoutSink};
    ...
    #[test]
    fn test_bridge_implements_runtime_observer_via_stdout() {
        let mut sink: Box<dyn OutputSink + Send> = Box::new(StdoutSink::with_is_tty(false));
        forward_text(&mut sink);
    }

    #[test]
    fn test_stdout_sink_default_creates_valid_instance() {
        let sink = StdoutSink::default();
        assert!(sink.spinner_handle.is_none());
        assert!(sink.pending_tools.lock().expect("lock").is_empty());
        assert!(sink.deferred_detail_lines.is_empty());
    }

    #[test]
    fn test_on_system_non_tty_doesnt_panic() {
        let mut sink = StdoutSink::with_is_tty(false);
        sink.on_system("System message: compaction triggered");
    }

    #[test]
    fn test_on_turn_finished_ok_non_tty() {
        let mut sink = StdoutSink::with_is_tty(false);
    ...
    #[test]
    fn test_on_turn_finished_err_non_tty() {
        let mut sink = StdoutSink::with_is_tty(false);
        sink.on_turn_finished(&Err("timeout".to_string()));
        assert!(sink.pending_tools.lock().expect("lock").is_empty());
    }
}
