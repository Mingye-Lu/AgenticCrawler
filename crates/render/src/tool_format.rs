...
#[must_use]
pub fn tool_input_summary(name: &str, input: &str) -> String {
...
#[must_use]
pub fn format_tool_start_line(name: &str, input: &str) -> ToolLine {
    ToolLine {
        icon: "\u{280B}",
        name: name.to_string(),
...
#[must_use]
pub fn format_tool_success_line(name: &str, input_summary: &str, output: &str) -> ToolLine {
...
#[must_use]
pub fn format_tool_error_line(name: &str, error: &str) -> ToolLine {
    ToolLine {
        icon: "\u{2717}",
        name: name.to_string(),
...
mod tests {
    ...
    #[test]
    fn tool_input_summary_with_invalid_json() {
        let s = tool_input_summary("bash", "not valid json");
        assert_eq!(s, "not valid json");
    }

    #[test]
    fn format_success_non_bash_multiline_output_collapsed() {
        let output = "first line\nsecond line\nthird line";
        let line = format_tool_success_line("navigate", "", output);
        assert_eq!(line.icon, "\u{2713}");
        assert!(line.summary.contains("first line"));
        assert!(line.summary.contains("second line"));
        assert!(!line.summary.contains('\n'));
    }

    #[test]
    fn format_bash_success_both_stdout_and_stderr() {
        let output = r#"{"stdout":"output line","stderr":"warn line"}"#;
        let line = format_tool_success_line("bash", r#"{"command":"cargo build"}"#, output);
        assert_eq!(line.icon, "\u{2713}");
        assert!(line.summary.contains("cargo build"));
        let stdout_lines: Vec<_> = line
            .detail_lines
            .iter()
            .filter(|l| !l.starts_with("stderr:"))
            .collect();
        let stderr_lines: Vec<_> = line
            .detail_lines
            .iter()
            .filter(|l| l.starts_with("stderr:"))
            .collect();
        ...
        assert!(stderr_lines[0].contains("warn line"));
    }
}
